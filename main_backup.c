...
int	mini_sort(t_stack **head_to_sort)
{
	...
	// il faut trouver le plus grand dans les trois,
		est-ce qu'il est au debut de la liste ? mais si il est au second emplacement ?
	// assume qu'il etais en haut, on l'envoi en bas avec rotate,
		maintenant on sait que le plus grand est en bas
...
void	set_push_cost(t_stack *head_a, t_stack *head_b)
{
	...
	while (temp_b)
	{
		if ((temp_a->is_o_med && temp_b->is_o_med) || (temp_a->is_o_med == 0
				&& temp_b->is_o_med == 0))
		{
...
void	move_node(t_stack *cheapest, t_stack **head_b, t_stack **head_a)
{
	...
	if (cheapest->is_o_med == cheapest->target_node->is_o_med)
	{
		...
		common_moves = (cheapest->med_index < cheapest->targ_med_index) ? cheapest->med_index : cheapest->targ_med_index;
...
/*
	...
	un count_push_cost = le push cost peut etre defini part le nobre de rotate nécéssaire pour monter le noeud en haut de la liste
		+ le nombre d'operation nescessaire pour que le head de a soi au bon emplacement
